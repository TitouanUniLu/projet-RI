...
pipeline_v3 = Pipeline([
    ...
    ('classifier', LogisticRegressionCV(cv=5, scoring='f1', n_jobs=cores, verbose=1))
])

    # Define the parameter grid for grid search
param_grid_v3 = {
        'sampling': [ 
                    SMOTE(sampling_strategy='minority', random_state=0, k_neighbors=5)],  # RandomOverSampler(sampling_strategy='minority', random_state=0),
...
if train:
    ...
    grid_search = GridSearchCV(estimator=pipeline_v3, param_grid=param_grid_v3, cv=5, scoring='f1', n_jobs=cores, verbose=1)
